...
- 投資內容沿用方格子紅線：免費、教育、方法論；不喊單、不給目標價、不做當期個股建議；每篇投資文的免責由文章模板無條件渲染（`src/pages/blog/[slug].astro`／`en/` 版對 `category==='investing'` 直出合規段），文末不另手打免責段（鐵律 10★★，2026-08-15）。
- 答案先行（2026-09-22，AEO）：每篇在開篇引言之後、第一個 `##` 之前放一段 2～4 句結論段，寫出主詞（節目／講者／公司）、日期、關鍵數字與限制，離開全文也讀得懂；結尾「## 帶得走的一件事」照舊。寫法細則見 `blog-admin` skill 鐵律 10。純文字版（`/blog/xxx.md`）開頭自動帶「來源：Realpha 讀市場（blog.getrealpha.com）」與該篇正式網址，由 `src/lib/content.ts` 的 `markdownForPost` 產生。
