...
![A line chart where two lines leave the same point, one rising and one falling, then reverse after the dashed liquidation line, and diverge further after the forced-close line: the long side accelerates down while the short side accelerates up, losing on both sides at once.](/figures/deleverage-both-sides-lose-en.svg)
...
![Two panels: on the left five bars of differing heights and directions stand for the diversification you assume; on the right the same five all fall together by similar amounts, because they sit in the same leveraged accounts.](/figures/fake-diversification-en.svg)
...
![Two bars of equal length: the upper one is a large position where only a sliver at the far left is marked as sellable in a day and the rest is stuck; the lower one is a small position that can be cleared end to end.](/figures/position-size-liquidity-en.svg)
