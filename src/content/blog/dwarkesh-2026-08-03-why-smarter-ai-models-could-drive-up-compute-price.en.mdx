...
![Two curves from the same starting point: the upper one climbs fast and high while the lower one stays almost flat, with the area between them filling out into a gap that widens to the right.](/figures/revenue-ten-times-compute-three-times-en.svg)
...
![A block marked gap on the left branches into three routes; the top and bottom routes are crossed out at their ends and only the middle one runs through, highlighted.](/figures/three-exits-two-crossed-out-en.svg)
...
![Two bars far apart in height: the very short one on the left is today's rent, and the one on the right, fifteen times higher, is the price implied by a human engineer's salary.](/figures/one-card-two-prices-en.svg)
...
![Three bars of different length stand for the three sources of the annual tripling, and the longest one runs into an upright wall at its end.](/figures/three-times-broken-into-three-sources-en.svg)
...
![On the left five blocks each stand on their own thin pillar; on the right the same five all rest on a single thick pillar.](/figures/many-names-one-pillar-en.svg)
