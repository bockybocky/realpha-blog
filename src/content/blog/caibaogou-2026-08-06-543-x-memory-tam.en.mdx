...
![On the left, wafer consumption in blocks: one for standard DRAM and four for HBM. On the right, gross margin bars: standard DRAM high and HBM noticeably lower, so it eats more and earns less.](/figures/hbm-wafer-cost-vs-margin-en.svg)
...
![On the left a block of externally granted opportunity pushes right with two arrows; a dashed threshold in the middle reads capital sunk, and only past it does the right-hand block become your own, each side listing what to track.](/figures/external-tailwind-to-own-moat-en.svg)
...
![One starting point splits into three branches: the top two converge on the same conclusion that supply is simply short, while the third, drawn as a dashed line in another colour, leads to the opposite conclusion that demand is weakening. A line underneath marks the unstated premise.](/figures/branches-all-point-up-en.svg)
...
![Two bars of very different height, the old forecast low and the new one high, with one arrow pointing up to a bigger market and a dashed arrow pointing back to the old bar, noting the same source was wrong by a factor of four two years ago.](/figures/tam-upgrade-two-readings-en.svg)
