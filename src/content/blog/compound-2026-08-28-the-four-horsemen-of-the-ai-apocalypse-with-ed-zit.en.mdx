...
![Two bars of equal length each split about seventy-thirty; the large block on the left is a handful of buyers, the small block on the right is every other customer, and both bars split at the same place](/figures/demand-concentration-two-companies-en.svg)
...
![On the left, twelve solid bars of equal height stand for contracts that renew every year; on the right only the first is solid, the eleven behind it are dashed empty bars, and the actual second month is only a short stub](/figures/arr-versus-run-rate-en.svg)
...
![Four nodes form a closed circle with arrows joined head to tail all the way back to the start; outside the circle only one dashed arrow with a question mark points in](/figures/circular-money-flow-en.svg)
...
![Two bars of very different height: the short one on the left is Amazon's twelve years of company-wide capital spending, the one on the right, nearly nine times taller, is one company's spending in a single year today](/figures/capex-scale-then-and-now-en.svg)
...
![Two rectangles of the same height, the right one stretched more than twice as wide so its area is clearly larger; height is labeled unit price, width is labeled usage](/figures/same-price-bigger-bill-en.svg)
...
![A two-by-two grid with the thesis right or wrong on one axis and the timing right or wrong on the other; only the entire top row is marked as getting paid](/figures/thesis-right-timing-right-en.svg)
...
![Two rising lines start from the same point, one climbing noticeably steeper, and the gap between them widens to the right and is filled in](/figures/receivables-outrun-revenue-en.svg)
...
![On the left a three-tier funnel narrows step by step downward; on the right a block with clear edges and a fixed size](/figures/granularity-of-a-claim-en.svg)
...
![A sheet split into two columns: the left has only two entries filled and a third slot still empty, marked by a dashed threshold line, while the right column is full](/figures/outside-inside-tally-en.svg)
