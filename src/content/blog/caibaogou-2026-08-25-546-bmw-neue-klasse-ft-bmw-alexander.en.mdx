...
![A small block on top stands for "one carmaker interview" and a much wider block below stands for "choosing on a user's behalf between global targets and local needs"; an arrow links the two layers and the lower one is clearly much wider](/figures/two-layers-under-a-brand-interview-en.svg)
...
![Two overhead road views; on the left a wide lane with only two cars, on the right a narrow alley with scooters packed around the car, leading to optional and standard equipment respectively](/figures/road-width-decides-the-standard-kit-en.svg)
...
![On the left twelve small boxes to be ticked one by one, on the right a single pre-bundled block of equipment, contrasting how many decisions each takes](/figures/many-choices-are-a-cost-en.svg)
...
![On the left one car carries four stacked job labels, on the right one car carries only one, using stack height to show the difference in load](/figures/how-many-jobs-one-car-carries-en.svg)
...
![Three overlapping circles for the economy car, the luxury sedan and the pure sports car, with a gray dot inside each circle and a blue dot at the point where all three overlap](/figures/three-circles-and-the-empty-middle-en.svg)
...
![On the left the body dips at the nose, marked as dipping; on the right the body stays level, marked as not dipping; a long bar below shows that 98% of braking comes from regeneration](/figures/does-the-nose-dip-when-it-stops-en.svg)

![Two side views; on the left the line of sight drops sharply from the eyes to the center screen, on the right it sweeps a strip of information almost along the direction of the road](/figures/where-the-eyes-have-to-go-en.svg)
...
![The top row of three cards has nothing beneath them; each of the three cards in the bottom row carries a hook, standing for a handle you can check](/figures/sentences-with-and-without-a-handle-en.svg)
...
![One starting point forks into two lines, the upper one differentiation and the lower one simply wrong, with a dashed line marking the boundary between before and after](/figures/same-signal-two-branches-en.svg)
...
![A rising certainty line crosses a falling line of remaining return, with the crossing point marked as the moment rivals start copying](/figures/certainty-crosses-the-room-left-en.svg)
...
![Two bars of equal height stand for the new-car denominator in each place; a sizable part of the top of the European bar is the additional car, while almost the whole Taiwan bar is the only car](/figures/the-two-denominators-en.svg)
...
The part that stayed with me most was the guest's test-drive tip: at a red light, watch whether the nose dips. A claim that specific can be caught out if it's wrong — and that's exactly why I trusted him more. Since then, when someone talks, I catch myself looking for the handle you could check. "We put the driver at the center" has no handle. "The nose won't dip when it stops" does.

Here's a small thing I tried, if you want to try it: go back over this week and find one thing someone told you that you nodded along to. Pull out one sentence with a handle — a date, a threshold, a result you could see. Then, before you leave work today, actually go check it once: open the report, make the call, or just stand at a corner and watch a car come to a stop. Afterwards write one line in your phone: that sentence, hit or miss. The first time I did this, the awkward part wasn't that the claim missed. It was that I went through the whole conversation and couldn't find a single sentence I could check. All of it was position. Coming up empty is a result too; note it down and leave it there.