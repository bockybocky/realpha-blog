...
![Four horizontal process-zone bands with a zigzag line shuttling between them and returning again and again to the lithography band at the top; the right side notes 8 to 10 miles traveled, 250 tools passed, and 12 returns to lithography](/figures/wafer-zigzag-path-en.svg)
...
![Two horizontal range bars on the same price axis; the cleanroom floor range overlaps heavily with the range for prime Taipei apartments](/figures/cleanroom-floor-vs-taipei-en.svg)
...
![Three fab cross sections side by side; in the first two the vehicles run on the floor and take up floor space marked in orange, in the third the vehicle hangs from a ceiling rail and lowers to the tool, leaving the floor clear](/figures/agv-rgv-oht-cross-section-en.svg)
...
![Three paired bar groups; in each the 200 mm bar on top is short and the 300 mm bar below is long, and the first group's long bar crosses a dashed line marking the human ergonomic limit](/figures/300mm-three-limits-en.svg)
...
![Three lines share one vertical generation-transition divider; after it the value of old patents drops steeply, R&D spending rises steeply, and customer switching speed falls](/figures/generation-shift-squeeze-en.svg)
...
![The left panel draws six tools each with three waiting blocks behind it; the right panel draws four tools each with only one waiting block, showing the shape difference between two ways of expanding](/figures/more-machines-vs-faster-flow-en.svg)
...
![A four-year horizontal schedule with three consecutive bars: building construction, a two-year transport system installation, and interface integration, with the start-of-production line at the far right](/figures/groundbreaking-to-production-en.svg)
...
![A vertical generation-transition line; the patent bar above stops dead before it, while the capability bar below passes through it and extends past three stops: steel, autos and wafers](/figures/moat-spec-vs-capability-en.svg)
...
![A long horizontal line standing for three weeks, cut into three stretches of waiting, with only a short segment near the right end being the twenty minutes of actual work](/figures/your-own-waiting-line-en.svg)
