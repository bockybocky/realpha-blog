...
![On the left, six separate small boxes each holding only two or three cells; on the right, one large box enclosing twelve cells at once](/figures/nuclear-buyer-orderbook-en.svg)
...
![A thick pipeline running left to right whose second section suddenly narrows to a thin line, while every other section keeps its normal width](/figures/nuclear-fuel-chain-bottleneck-en.svg)
...
![A bar standing for all the heat, with one slice taken from the front, a smaller slice taken right after it, and a long remaining section that is still rejected waste heat](/figures/bottoming-cycle-extra-slice-en.svg)
...
![A clockwise closed loop of four nodes, with an external feed arrow at the upper left and a dashed line to the outside market on the right crossed out](/figures/tritium-closed-loop-en.svg)
...
![An almost flat line on the left, a square wave jumping rapidly between high and low on the right, and a box between them](/figures/power-shape-grid-vs-datacenter-en.svg)
...
![Three bars of different length on one timeline, all ending inside the same boxed interval](/figures/fission-fusion-timeline-overlap-en.svg)
...
![Five equal cells on the top row and five cells of very different widths below, with the third one especially long](/figures/planned-vs-actual-steps-en.svg)
