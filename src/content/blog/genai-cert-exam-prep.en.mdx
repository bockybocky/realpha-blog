...
![A large block on the left holds seventy-one dense points of which twenty-one are dark; after an arrow in the middle marked AI comparing against the questions already practised, only a small block of twenty-one dark points remains on the right.](/figures/compress-the-question-bank-en.svg)
...
![Two circles for text and speech, an arc arrow from text to speech above and another from speech back to text below, plus a loop arrow on the right that returns to itself, giving three different directions.](/figures/speech-three-directions-en.svg)
