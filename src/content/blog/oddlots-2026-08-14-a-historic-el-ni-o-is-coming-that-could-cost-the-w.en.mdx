...
![Two cross-sections of the Pacific: on the left, normally, the trade winds pile a thick warm pool against the western side; on the right, in an El Nino, the trades weaken and the same warm water spreads into a thin layer across the whole basin.](/figures/el-nino-warm-pool-spreads-en.svg)
...
![A line chart where a grey dashed line is the growth path without an El Nino; the orange line is the assumed path, which dips and returns to that path, while the blue line is the actual one, which dips and then follows a lower slope, so the gap between them widens to the right.](/figures/level-effect-vs-growth-effect-en.svg)
...
![Two panels compared: on the left the disaster markers on four tracks fall at different times and the total below is only four small bumps, while on the right all four line up at the same moment and the total becomes one tall bar.](/figures/simultaneity-breaks-dilution-en.svg)
...
![A row of cells for years with a few marked as El Nino years and the rest as other years; each group draws a bar below comparing subsequent growth, and the El Nino bar is clearly shorter.](/figures/counterfactual-is-the-other-years-en.svg)
...
![Four boxes for different industries along the top look independent, but four lines run down and converge on one base marked with the underlying variable they share.](/figures/diversification-shared-variable-en.svg)
...
If this episode leaves you with one idea, make it this: **diversification is not how many different things you hold. It is a claim about what would have to happen for them to break together.**

El Niño is expensive not because it is unusually violent but because it fires in many unrelated places at once — heat, drought, flood, fire, all in the same season. Normally the system dilutes local disasters. That dilution *is* diversification. When it fails, you learn the spread was never real.

Here is a practice for this week, and it requires no market data. Write down three things in your life you could not easily replace — a source of income, the place you live, the one relationship your whole social world runs through. Then try to finish this sentence: **is there a single event that would make all three worse at the same time?**

If you can write it, you now know the shape of your real risk and can decide whether to keep carrying it. If you cannot, there are two possibilities — you are genuinely spread out, or you simply have not thought of it yet. Keep those apart, because what you do next is completely different.
