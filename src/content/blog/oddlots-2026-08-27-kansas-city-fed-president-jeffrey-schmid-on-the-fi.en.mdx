...
![Two payment flows compared; the upper one has a wide float and fees block in the middle, the lower one has that middle replaced by an empty dashed box crossed out, with duration and liquidity blocks rising from below to hold up the gap](/figures/atomic-settlement-buffer-swap-en.svg)
...
![Two borrowers at the top each run an arrow into the same pool of funds in the middle, with a rising price scale to the right of the pool and a note below that demand exceeds supply](/figures/two-borrowers-one-pool-en.svg)
...
![A thick flow of resources on the left splits in two; the branch to data centers stays thick, the branch to other industries is squeezed thin, with longer lead times and higher costs marked beside the thin branch](/figures/crowding-out-thick-thin-en.svg)
...
![Two axes of very different length; the long upper one connects plants and production lines and is marked slow to react, the short lower one connects inventory and credit lines and is marked fast, with a crossed-out dashed line between them showing the short end barely reaches plant building](/figures/long-end-short-end-decisions-en.svg)
...
![Two parallel lines on one time axis; the upper turns down on the day of the event, the lower not until a much later decision cycle, with the time gap marked between the two turning points](/figures/valuation-vs-operations-lag-en.svg)
...
![One block of demand at the center splits down into two sides, the left marked with a plus as the side collecting money and the right with a minus as the side paying it, with five dots at the lower right all crowded onto one side and labeled false diversification](/figures/same-demand-opposite-signs-en.svg)
...
![A clockwise loop of five nodes: buildout boom to commodity and wage costs, to sticky inflation, to the long end and discount rates, to pressure on the valuation of distant profits, with a note in the center that the same variable appears twice](/figures/boom-suppresses-itself-loop-en.svg)
...
![One set of data at the top connects down to two scales, the left one tipping toward inflation and the right one tipping toward employment, the two tilting in opposite directions](/figures/same-data-different-ranking-en.svg)
...
![A line jumping violently up and down on the left stands for tone, a steady sloping line on the right stands for the reaction function, with a dashed line lying exactly on top of it to show tomorrow won't change it](/figures/tone-noise-vs-reaction-function-en.svg)
...
![Two identical timelines; the upper has a wide buffer band in the middle with the cost block at the far right, after the fact, and the lower has the buffer squeezed to a line with the same cost block moved to the far left, before the fact](/figures/cost-moves-from-after-to-before-en.svg)
