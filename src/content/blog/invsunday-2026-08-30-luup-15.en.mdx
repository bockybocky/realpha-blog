...
![A curve that dips deeply then rises sharply, set against an almost flat gray line; the dip is labeled looks like waste, and most people leave at the bottom](/figures/short-term-unreasonable-long-term-reasonable-en.svg)
...
![Four nodes arranged in a ring with arrows running around it: startups grow businesses, large firms acquire them, talent moves into the large firms, and later they use startups as engines](/figures/japan-startup-loop-en.svg)
...
![Two lines diverging on the same timeline: the line for talent keeps rising while the line for pay stays nearly flat, opening a clear gap at the right end](/figures/talent-up-pay-flat-en.svg)
...
![Two panels compared: on the left a dashed box holding hollow squares for conditions that cannot be confirmed, on the right a solid box holding filled squares for behavior that has already happened and can be checked](/figures/condition-layer-versus-behaviour-layer-en.svg)
...
![Four investment points on a timeline, each labeled another look below, with the flag for the law changing landing after the fourth](/figures/four-rounds-before-the-rule-changed-en.svg)
...
![A funnel screens a whole row of candidates down to four of exactly the same size; below the funnel those four grow into bars of wildly different heights](/figures/elimination-filter-loses-discrimination-en.svg)
...
![Two companies each with a stacked bar before and after pressure; one cuts its marketing segment, the other cuts R&D, and the totals are the same](/figures/what-they-cut-first-en.svg)
...
![A wildly swinging curve whose trough is marked by a horizontal dashed line labeled the end you imagined, while the curve keeps running up to the right](/figures/the-lowest-point-is-not-the-end-en.svg)
...
![A ruler on each side: the left one measures the drawdown on paper with its arrow pointing at a person, the right one measures how many reasons to buy are left with its arrow pointing at a company](/figures/two-rulers-measuring-different-things-en.svg)
...
![Two sheets side by side, each split into a left and a right column; on one the left column is full and the right blank, on the other the right column is full, with a verdict line under each sheet](/figures/two-columns-condition-and-behaviour-en.svg)
