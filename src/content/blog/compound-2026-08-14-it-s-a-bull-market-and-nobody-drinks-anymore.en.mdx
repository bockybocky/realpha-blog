...
![Five same-theme funds along the top all connect to the same set of holdings in the middle, and underneath there are only three actual companies.](/figures/wrappers-vs-underlying-en.svg)
...
![One hundred cells stand for the whole index: staples take only four and a half, discretionary under ten, and the remaining eighty-odd are everything else.](/figures/defensive-shrinking-en.svg)
...
![The index falls twenty percent then rises twenty-five to return to one hundred, while a two-times leveraged product falls forty then rises fifty and only reaches ninety, leaving a ten percent gap at the end.](/figures/leverage-decay-en.svg)
...
![A three trillion dollar cash pool, twice the level of six years ago, with both gates to the stock market still shut: one is rates below three percent, the other a drawdown of more than thirty percent.](/figures/three-trillion-gates-en.svg)
...
The line that stuck with me most was the photonics example: five funds, three companies underneath. That sentence is what sent me back to my own subscriptions and accounts. Different names everywhere — and laid out side by side, several of them were the same bet, while I'd been calling that diversification.

One small thing I did once, for what it's worth: go back to three things you've added in the past three months — a fund, a subscription, a tool, whatever. For each one, write two lines. First: "which gap of mine did this fill." Second: "which thing I already had does it overlap with." The one where the first line won't come but the second writes itself is the one you found first and invented a purpose for afterwards. Then write one number in the corner: how many of the three are like that. My count was two — and I'd proudly recommended one of them to a friend.

If you want to go one step further: next time, before you hit confirm, write that first line before you press.