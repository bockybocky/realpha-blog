...
![Two rows of bars: the intelligence scores of three models along the top are nearly equal in height, while the cost per task below differs sharply, the dearest more than four times the cheapest.](/figures/frontier-models-score-vs-cost-en.svg)
...
![A row of sixteen cells stands for sixteen words guessed at once by the draft model; after one pass by the large model the first eleven are filled in and accepted, the twelfth is crossed out and the rest are discarded as dashed cells.](/figures/speculative-decoding-draft-model-en.svg)
...
![Two panels compared: the left is scattered with dots for unsolved problems and has a long marginal-contribution bar, while the right has only three dots and the bar has shrunk to a stub.](/figures/marginal-contribution-two-timepoints-en.svg)
...
![Three bars of different length stand for how long each of three things would stall if you vanished, the two shortest marked as easily covered and the longest marked as scarcity.](/figures/how-long-would-it-stop-en.svg)
