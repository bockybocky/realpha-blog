...
![A bar comparison: three years ago the cost bar stood above the dashed line marking what the client would pay, so the matter never happened; after AI the cost bar is a third of that and drops below the line, so the matter goes ahead.](/figures/jevons-cost-threshold-unlocks-work-en.svg)
...
![Two crossing lines: one rising to the right for the value of an hour, one falling to the right for the hours the same matter takes, with the result on the right that the client's total falls while the lawyer's hourly rate rises.](/figures/hourly-rate-up-total-bill-down-en.svg)

## Where my thinking goes

**1. I am a walking sample of the Jevons Paradox.** I run a one-person company. AI didn't shrink my workload — it moved everything from "impossible for one person" onto my calendar: a self-built market database, a research library, a multi-model engineering team. Efficiency up → ambition up → total work up. True for lawyers, true for individuals; I suspect it will be true for corporate IT budgets.
