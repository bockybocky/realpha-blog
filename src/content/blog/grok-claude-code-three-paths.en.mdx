...
![A four-quadrant map: left to right runs from grey area to officially sanctioned, top to bottom from same window to a separate window. Subscription relay sits upper left, API key upper right, the official command-line tool lower right, and the lower left quadrant is empty.](/figures/grok-three-paths-positioning-en.svg)
...
![Two timelines one above the other, both hitting a credential expiry at the same moment. On the top one a person is present and the line carries on afterwards; on the bottom one nobody is watching, so the line goes dashed all the way to morning, with a long unrecoverable window marked below.](/figures/grok-unattended-vs-attended-en.svg)
