...
![Two groups of bars: on the left, an ordinary society, four bars of equal height; on the right, a mafia state, four bars rising from left to right, shortest for the critical professor and tallest for the proxy fighter abroad, with loyalty to the system along the horizontal axis.](/figures/loyalty-replaces-citizenship-as-the-axis-en.svg)
...
![A growth curve rising to the right hits a horizontal dashed threshold and forks: a dashed branch continues up but has a share taken away, while the other turns flat and stops below the threshold.](/figures/the-ceiling-on-success-en.svg)
...
![Two roads to the same destination: the upper one has a single checkpoint at a fixed place charging a fixed amount, while the lower one has several checkpoints in shifting places plus a dashed one for tomorrow, and the road turns away at the end.](/figures/priceable-cost-versus-unpriceable-chaos-en.svg)
...
![Three panels in a row, each with a grey bar standing for oversight; the gap in that bar goes from very narrow to moderate to nearly the whole length, and what passes through grows from a wheelchair to unchecked imports to an entire port.](/figures/the-exception-widens-en.svg)
