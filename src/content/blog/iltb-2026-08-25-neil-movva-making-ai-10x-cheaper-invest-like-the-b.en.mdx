...
![Above, a short timeline chopped into eight segments, standing for you typing and it replying while you wait through the whole thing; below, one long continuous bar, standing for it running for hours after you have left; at the bottom two proportion bars show the background share growing from half to ninety percent](/figures/waiting-disappears-en.svg)
...
![On the left two routes: above, one car carrying one person straight there; below, a road with four stops but a full load. On the right two bars: the cards put in are eight times taller, the speed gained only four and a half times, with the gap labeled as eaten by communication](/figures/throughput-vs-latency-en.svg)
...
![An uptime scale descending from one hundred percent; the very narrow band at the far left is where everyone is bidding, and the wide stretch to the right is his own buying zone](/figures/uptime-buyer-line-en.svg)
...
![On the left a fixed pool packed with small squares standing for the one-off subsidy of internet text, with an arrow to two new sources on the right: above, a loop of verifiable environments; below, the unverifiable half left to people](/figures/data-stock-to-flow-en.svg)
...
![Three horizontal bars of very different lengths standing for how much is wasted: compute is short, memory cache and idle cards are much longer](/figures/where-the-waste-is-en.svg)
...
![In both small charts shipments are rising; on the left the price line is flat, on the right the price line turns up with them](/figures/tight-supply-vs-pricing-power-en.svg)
...
![On the left a box where things go in and straight out, almost empty; on the right a box packed full with only a dashed exit leading to users who have not arrived](/figures/consumption-vs-speculation-en.svg)
...
![A threshold line moving down from a high position, taking in a large field of ideas that were shut out below it; that field is the set of attempts whose value is still unproven](/figures/threshold-drops-unlocks-en.svg)
