...
![A block on the left for the same open-weight models and accelerators splits into two bars of very different length: the upper short one marked one times and the lower one five times as long, showing the same input producing a fivefold gap.](/figures/same-hardware-five-times-faster-en.svg)
...
![Five rows from top to bottom stand for cloud, compute, inference, chips and applications; each row holds nine dots, of which only one is a large solid winner and the other eight are small grey dots that fell away.](/figures/every-layer-wins-few-companies-en.svg)
...
![Two panels compared: on the left a tall grey wall blocks the way between the old and new database, on the right only a stub of that wall remains and the arrow passes straight through, with the deciding factor below shifting from stickiness to iteration speed.](/figures/migration-wall-collapses-en.svg)
...
![A smooth grey rising curve for human ability beside a violently jagged blue line for model ability, with one of its peaks marked as a spike and one of its troughs as a valley.](/figures/jagged-capability-frontier-en.svg)
...
![A banner at the top marks the reasoning as correct; a long bar in the middle stands for a day's workload of which only a small stretch at the far left is filled as the narrow task it can do; below, an arrow from model reading toward replacing the doctor is stopped by an upright wall marked reimbursement and liability.](/figures/hinton-two-missing-layers-en.svg)
