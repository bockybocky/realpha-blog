...
![Two lines leave the same point, rising then falling, and both end below where they started after two days, with the two-times leveraged line falling further.](/figures/leverage-volatility-decay-en.svg)
...
![Two lines cross over five years: China's weight slides from the top down to 19% while Taiwan climbs from below to 27.3%.](/figures/em-index-weight-swap-en.svg)
...
![Two lines cross: free cash flow at the cloud giants slides toward zero while upstream hardware climbs.](/figures/free-cash-flow-shift-en.svg)
...
![On the left, dashed bars sketch the expectation that active should be higher; on the right, the two actual bars are the same height.](/figures/active-vs-passive-control-group-en.svg)
...
![Three seemingly different holding blocks all send arrows down onto the same supply-chain bar.](/figures/exposure-overlap-passthrough-en.svg)
