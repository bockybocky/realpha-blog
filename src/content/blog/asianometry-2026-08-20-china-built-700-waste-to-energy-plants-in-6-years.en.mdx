...
![Two bars compare steam temperature; the waste-to-energy bar is shorter and capped by a dashed ceiling line, while the coal bar is clearly higher](/figures/steam-temperature-ceiling-en.svg)
...
![Two revenue bars against a dashed cost line; the left bar with subsidy clears the cost line, the right bar with the subsidy stripped out falls below it](/figures/subsidy-half-the-revenue-en.svg)
...
![A container representing national incineration capacity; the solid lower part is the waste actually received each day, and the top 40% is empty and labeled as such](/figures/capacity-versus-garbage-en.svg)
...
![Dots on a utilization axis split into a low cluster and a high cluster with almost nothing between them, and the average line falls right in the empty gap](/figures/utilisation-two-clusters-en.svg)
...
![Two timelines; on the upper one the objection arrow lands after the announcement and the project is abandoned, on the lower one it enters during design and the project reaches groundbreaking](/figures/when-objection-enters-en.svg)
...
The line from this episode that stuck with me hardest is "the subsidy isn't an accelerant, the subsidy is the revenue." It's the reason I started pulling apart, one by one, the money in my own life that I'm not the one paying — the employer's top-up, the hours a family member absorbs, the reach a platform still gives away for free. They arrive so naturally I had never entered them as a cost.

The method is simple and costs nothing. Tonight, open last month's statement or calendar, point at one thing you do on a fixed basis, and fill in two numbers next to it: what share of it someone else is paying (money, time, or patience all count), and how many fewer times a month you'd do it if that share went to zero tomorrow. The first number came to me fast; the second took ten minutes, because it had never occurred to me that the thing could be done less.

Keep the sheet. The day next month's statement arrives, take it out and check whether those fewer times actually happened.