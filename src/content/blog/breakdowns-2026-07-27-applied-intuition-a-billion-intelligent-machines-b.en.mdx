...
![Three stacks left to right, each one layer taller than the last, with the dashed box marking the current bottleneck pushed one layer higher each time, forming a rising staircase.](/figures/bottleneck-moves-up-the-stack-en.svg)
...
![Two bars of very different length on the same year axis: phones and laptops are only a short stub, while cars and machines stretch across the full fifteen years.](/figures/diffusion-friction-two-timelines-en.svg)
...
![The top row is the textbook drawing, a large circle with a small one right beside it. The bottom row is drawn to scale: the same large circle sits at the far left and the dot standing for Earth is pushed to the far right, with a wide empty gap between them.](/figures/textbook-scale-versus-real-scale-en.svg)
...
![On the left a single vertical path runs from technology to machine to market; on the right one technology block fans out to five customer blocks in different industries, a line against a fan.](/figures/vertical-versus-horizontal-payoff-en.svg)
...
![Six seemingly separate industry blocks across the top, all six lines converging into one block below, which then splits into two opposite outcomes.](/figures/six-positions-one-assumption-en.svg)
...
![A four-quadrant chart with technology settled on one axis and business model settled on the other. A point in the lower left marks the early assessment of robotaxis, and a point in the upper right marks the moment they decided to build.](/figures/tech-settled-model-settled-quadrant-en.svg)
