...
![Two panels compared; on the left the asset line falls and the spending line is flattened for five to seven years, on the right the asset line rises and the spending line rises with it](/figures/two-kinds-of-resilience-en.svg)
...
![Two time scales; the car loan one stops at sixty days without crossing the red line at a hundred and twenty, and the gas one is left unpaid through summer and made up before winter](/figures/calculated-delay-two-rulers-en.svg)
...
![A factory on the left sends many lines out into the community, while the data center on the right connects only two people lines and one tax line](/figures/no-political-base-en.svg)
...
![Bars extending left and right from a center axis; multifamily housing and offices shrink to the left while industrial and data centers grow to the right, with an arc marking where workers and transformers moved](/figures/construction-moved-house-en.svg)
...
![Two time bars of equal length; the upper is one color throughout, the lower is cut into two at the same point, with the cut marked the plane is about to land](/figures/sixty-five-versus-forty-seven-plus-eighteen-en.svg)
...
![A staircase descending from upper left to lower right, each step a level of ability to pass costs through, with the bottom step the shortest and marked nearly impossible](/figures/passthrough-staircase-en.svg)
...
![Two bars of different colors on the left add up to a single bar of the same height on the right, with labels below showing what holds each of the two up](/figures/aggregation-hides-mechanism-en.svg)
...
![Two bars of very different height on either side of a vertical dashed line marked same interest rate, with an arrow pointing right to where the real difference lies](/figures/same-variable-different-outcome-en.svg)
...
![A line runs to a shock point and splits in two, one returning to the original dashed track and one staying low, with the distance between them marked as the fork](/figures/the-fork-is-your-kill-switch-en.svg)
...
![Two paths to the same destination, a straight line above marked downwind and a zigzag below marked upwind](/figures/upwind-still-arrives-en.svg)
...
![The box on the left has a door in its wall and the arrow of evidence passes through; the box on the right is sealed and the arrow bounces off](/figures/judgment-has-a-door-en.svg)