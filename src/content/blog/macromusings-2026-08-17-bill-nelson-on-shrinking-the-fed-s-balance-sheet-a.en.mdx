...
![On the left the floor system: every bank deals only with the central bank, forming a star. On the right a market-first system: banks lend to each other in a mesh while the central bank steps back as a backstop.](/figures/floor-vs-interbank-en.svg)
...
![Two lines from the same point: SVB's plunges into a disorderly failure while Silvergate's slopes gently down to a self-managed wind-down.](/figures/two-bank-runs-en.svg)
...
![Two crossing lines: the more embarrassing it is to borrow from the central bank, the stronger the signal in that news, and the less resilient the system.](/figures/stigma-tradeoff-en.svg)
...
![The top row has plenty of assets but the route to cash is cut, while the bottom row has fewer assets and an open route with the arrow reaching cash.](/figures/liquidity-is-a-path-en.svg)
