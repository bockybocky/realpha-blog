...
![Two cards each holding a Chairman box and a CEO box; on the left decisions flow along a single arrow to the CEO, on the right they split into two, the thicker one running up to the chairman](/figures/chairman-title-vs-decision-flow-en.svg)
...
![Two lines rising to the right; the upper one climbs fast and high, the lower one is nearly flat, and the gap between them widens with time](/figures/hardware-outruns-software-en.svg)
...
![A full-width bar on the top row split into left and right halves for the company's two holes; below, the successor's strength fills only the left half, the right half empty, with another dashed box added underneath](/figures/successor-strength-and-the-gap-en.svg)
...
![A flat horizontal line on top for shipping and revenue, a line below sloping down to the right for the risk taken in design, and a dashed line on the right marking where it finally gets noticed](/figures/lagging-numbers-leading-design-en.svg)
...
![Two bars of very different lengths; the short one breaks into a dashed line early on and the long one runs all the way to the right edge](/figures/spec-outlives-nothing-reason-does-en.svg)
...
![Two rulers measuring the same distance: the upper one spans three years in three sections, the lower one covers only a short stretch near the start](/figures/three-years-measured-by-three-months-en.svg)
...
![In the left box four small cards; three follow solid arrows into the right box and the fourth falls along a dashed line onto a gray band at the bottom](/figures/the-thing-that-falls-on-the-floor-en.svg)