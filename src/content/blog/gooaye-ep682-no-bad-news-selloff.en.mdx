...
![Two panels: on the left a line rises steeply then flattens at a high level, and on the right four bars fall from high to zero, showing the level is still high while the rate of change has gone to zero.](/figures/level-versus-rate-of-change-en.svg)
...
![Two lines leave the same point in opposite directions: the upper one, standing for checkable numbers such as prices and results, keeps rising, while the lower one, the share price, slides, with the divergence marked at the right.](/figures/verifiable-numbers-versus-price-en.svg)
...
![Two panels draw the same horizontal moving average: on the left the price crosses up through it followed by a cross mark, meaning it has no predictive power as an entry signal; on the right the price breaks down through it and the break is marked as the exit, meaning it works as discipline.](/figures/moving-average-two-uses-en.svg)
