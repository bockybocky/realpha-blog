...
![A curve that is steep then flat: as assets rise from a low level the difference in daily life opens up fast, and past a certain level the curve is almost flat, with the wide area on the right greyed out as barely noticeable.](/figures/capacity-diminishing-gap-en.svg)
...
![Two panels compared: on the left one circle holds both cloud compute and frontier research; on the right the circle holds only cloud compute, with frontier research moved outside it but still joined by two lines marked investment and tenant.](/figures/research-moved-outside-rent-stays-en.svg)
...
![Two stacked bars compared: the left has cash flow below and a very tall option on top, while on the right cash flow is the same height and the option has shrunk to a sliver, with an arrow marked shrunk.](/figures/valuation-two-blocks-option-shrinks-en.svg)
...
![Two rows compared: the top row has a small block for a small position with only one news cell beside it, and the bottom row has a large block for full leverage with eight news cells beside it.](/figures/position-size-drives-news-intake-en.svg)
...
![A thick wall marked earnings stands at the right end of a timeline while three arrows push leftward in stages, moving the selling point from the release day to the day before, then earlier, until nobody dares to step in.](/figures/front-running-recursion-hits-the-wall-en.svg)
