...
![Two ten-cell blocks: next door and zip codes with no proposal both show seven opposed and three not, exactly the same ratio; the two bars below are of very different height, showing that only people living beside one rank it in their top five voting issues.](/figures/not-a-typical-nimby-en.svg)
...
![The same two boxes in two rows; in the top row a thick gray arrow runs from the town to the developer, in the bottom row a thick blue arrow runs the other way, exactly reversed.](/figures/bargaining-power-flipped-en.svg)
...
![The sentence "it's a bubble" branches into two paths: a thin gray dashed line on the left leading to nothing changed, and a thick blue solid line on the right leading to shorter contracts, prepayment, and refusing expansion, the two lines very different in weight.](/figures/bubble-talk-two-branches-en.svg)
...
![The same promise is one full bar at the top; below, after each company's delivery record is applied, one bar is left at a third of the length and the other keeps its full length, a clear gap.](/figures/discount-by-track-record-en.svg)
