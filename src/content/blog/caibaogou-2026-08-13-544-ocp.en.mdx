...
![Three bars stepping up in height, labelled rack, power hall and data centre, with now, next year and 2029 below, showing that the power roadmap has fixed both wattage and dates for the first time.](/figures/power-roadmap-three-steps-en.svg)
...
![Two bars on the left differ tenfold, standing for a hundred layers in standard parts and a thousand in speciality parts; on the right the same capacity yields ten standard units but only one speciality unit.](/figures/passive-component-crowding-out-en.svg)
...
![Two bars compared: copper used to run the whole distance, and now it covers only a short stub, with everything past two metres switching to optics.](/figures/copper-reach-two-meters-en.svg)
...
![Two bars of very different length on the same time axis: building from nothing runs long, while a shortage caused by capacity being used elsewhere is far shorter.](/figures/two-kinds-of-shortage-en.svg)
...
![Two curves: the scarcity level keeps rising but flattens, while the valuation curve turns down while the level is still high.](/figures/level-vs-acceleration-en.svg)
