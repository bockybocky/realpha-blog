...
![Three bars dropping sharply in height from left to right for pre-training, fine-tuning and RAG, where height stands for compute, showing they differ by an order of magnitude and change different things.](/figures/pretrain-finetune-rag-cost-en.svg)
...
![A large box holds two smaller ones: the outer box is prompt injection and the inner two are jailbreaking and prompt leaking, showing by containment that jailbreaking is one kind of injection.](/figures/prompt-injection-contains-jailbreak-en.svg)
...
![Three canvases side by side: the first has a dashed hole cut inside it, the second gains a dashed border outside the original edge, and the third grows from a small canvas to a large one, so the moving boundary tells the three edits apart.](/figures/inpaint-outpaint-upscale-en.svg)
...
![On the left the data points shift right as a group while the boundary stays put; on the right the points stay put while the boundary rotates, so the two drifts are told apart by what moves.](/figures/data-drift-vs-concept-drift-en.svg)
...
![A four-tier pyramid: the narrowest top tier is unacceptable risk and each tier below is wider down to minimal risk, so narrow-at-the-top means stricter and wide-at-the-bottom means more numerous.](/figures/eu-ai-act-risk-pyramid-en.svg)
...
![The left half is two blocks pointing arrows at each other in a loop; the right half is four frames going from full of noise to clean, contrasting adversarial generation with step-by-step denoising.](/figures/gan-vs-diffusion-en.svg)
...
![On the left the same word has one fixed point that both senses point to; on the right the same word splits into two points, one near river and one near money, so position carries the context.](/figures/word2vec-vs-bert-position-en.svg)

![Two identical two-by-two grids side by side: the left highlights a whole column and the right a whole row, showing the two metrics share a numerator but divide by different totals.](/figures/precision-recall-denominator-en.svg)
...
![Of eight expert cells only two light up, and two bars on the right, one tall and one short, compare total parameters with the compute actually used per query.](/figures/moe-partial-activation-en.svg)
...
![A U-shaped curve, high at both ends and sagging in the middle, with position in the conversation along the horizontal axis and how well it is remembered up the vertical.](/figures/lost-in-the-middle-curve-en.svg)
...
![Between the text block and the speech block at the top run a pair of opposite arrows, and below them another arrow runs from speech to speech, so the direction of the arrow tells the three terms apart.](/figures/tts-stt-s2s-directions-en.svg)
...
![Five parallel arrows run from your side to the same counterparty, each carrying something different: parameters, noisy statistics, ciphertext, shards, and fake data.](/figures/pets-what-leaves-your-side-en.svg)
