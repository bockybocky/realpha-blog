...
![A wide ore body on the left passes through an extremely narrow smelting bottleneck, leaving only a thin stream of usable metal on the right; two bars below compare the three US copper smelters with the forty-two China built in the past decade](/figures/ore-is-not-material-en.svg)
...
![Two panels side by side: the same three gray dots start below the economic threshold line on the left; on the right the line drops and the three dots rise above it, lit up](/figures/threshold-drops-stock-surfaces-en.svg)
...
![Top panel: an oil price line swings up and down. Bottom panel: a critical minerals price line runs flat, pinned below a dashed cost line, while a row of plant blocks on the right empties out one by one](/figures/not-a-normal-market-en.svg)
...
![Left panel: burned hydrocarbons scatter into dots that vanish. Right panel: metal that has been used has an arrow curving back to the start. Below, on an axis from zero to one hundred percent, only the ten to fifty band is marked, as a wide estimate range](/figures/urban-mining-unknown-range-en.svg)
...
![From one starting point two paths split: the upper path stays and cleans up, ending in a solid-outlined working example; the lower path leaves entirely, its middle a dashed blank, ending in an empty dashed box](/figures/two-industries-two-choices-en.svg)
...
![An axis running from I don't know to I'm certain: recycling share, permitting, and fusion sit on the left; oil and gas reserves and the solar share sit on the right. Below, a small block is blown up by an arrow into a large one](/figures/dont-know-versus-certain-en.svg)
...
![Two bars of very different length on the same year axis: the twelve-month bar is a short stub, while the bar for rebuilding smelting capacity runs past ten years and continues as a dashed line](/figures/moving-versus-talking-en.svg)
...
![A table already filled with three columns gains a new rightmost column, What this does for me; two of its cells have answers and two are empty dashed boxes](/figures/add-one-column-en.svg)
