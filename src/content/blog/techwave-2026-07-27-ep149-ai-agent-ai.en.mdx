...
![Four bars descending in height stand for task uncertainty falling from done for the first time to done many times; the first three carry ticks on top and the shortest one carries a cross.](/figures/flight-hardest-passed-easiest-failed-en.svg)
...
![Two timelines side by side: the upper one spreads its milestones thinly across five years, while the lower one crowds its first two at the far left and its dashed continuation ends at about three years.](/figures/milestone-ladder-two-generations-en.svg)
...
![On the left a five-step staircase rising from low to high stands for privileges climbing one level at a time; on the right a box holds dozens of small blocks packed together, standing for a mass of simultaneous attacks.](/figures/agent-privilege-escalation-ladder-en.svg)
...
![On the left a wall with a hole in the middle lets the arrow pass straight through; on the right an intact wall stops the arrow; and below runs a path with no wall at all where the arrow goes all the way.](/figures/guardrail-asymmetry-attack-defense-en.svg)
...
![Two parallel timelines: the upper one is sparsely marked with only five nodes, the lower one densely marked, and a vertical dashed line crosses both.](/figures/two-clocks-progress-vs-price-en.svg)
