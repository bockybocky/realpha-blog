...
![A histogram of company earnings growth: most bars cluster left of center with a low long tail to the right; the median dashed line sits in the middle of the distribution while the index-weighted dashed line is pulled further right by a few bars](/figures/earnings-index-vs-median-en.svg)
...
![Two lines over time: the cost line rises then falls back, the price line rises and stays flat, and a shaded gap opens between them on the right](/figures/tariff-ratchet-en.svg)
...
![Two paths, one above the other: the upper one runs to a dead end with a small buyer block and only three customer dots below it; the lower has a large buyer block with a dense field of customer dots below, plus an extra downward exit at the end](/figures/capex-cycle-then-vs-now-en.svg)
...
![Both ends of a bell curve are grayed out, the left tail labeled displaced and the right tail labeled missing out; below, eighty percent of a bar is taken by gray, leaving only the right fifth as public markets](/figures/small-cap-two-tails-en.svg)
...
![Three bars against a zero line: the contracting gross margin group falls deep, the flat group dips slightly, the expanding group rises](/figures/margin-compression-punished-en.svg)
...
![An earnings curve with two pop-up windows carrying close buttons floating above it, touching nothing; a third piece of news points into the curve with an arrow and bends the later stretch steeper](/figures/news-reaching-the-income-statement-en.svg)
...
![A long row of forty-five thin bars of equal height, and after a gap on the right, five much taller thick bars](/figures/position-sizing-structure-en.svg)
...
![Two small panels: the left line drops one big step and then runs flat at the low, the right line falls then recovers and runs off the top of the frame](/figures/cyclical-vs-structural-drawdown-en.svg)
...
![Two paths relative to the same baseline: on the left a narrow, deep, clearly bounded dip that returns quickly; on the right a line sloping gradually down and fading out as a dashed line](/figures/regulatory-vs-competitive-risk-en.svg)
...
![A two-by-two grid where the top-left and bottom-right cells are outlined solid as the right calls, while the other diagonal is labeled as two kinds of pain](/figures/temporary-vs-permanent-matrix-en.svg)
