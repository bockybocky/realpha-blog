...
![A pipeline running left to right: front-end capacity is high, a narrow throat sits in the middle, and actual shipments on the right are only as tall as that throat, with the extra above and below marked as unshippable. A dashed frame around the throat shows this section can still be widened.](/figures/back-end-bottleneck-caps-front-end-en.svg)
...
![Two panels: on the left money is spread across several small blocks and one medium one; on the right the small blocks are flattened and emptied while the middle block has grown tall and large and carries a crack.](/figures/crowded-safe-haven-before-after-en.svg)
...
![Three rectangles on a shared baseline where width is earnings, height is the multiple and area is the share price: the original large rectangle becomes either narrow and tall or wide and flat, both with the same smaller area, the first marked cut and the second marked wait.](/figures/earnings-times-multiple-two-shrinks-en.svg)
...
![Four cards in a row: the first three are struck through with the evidence that ruled them out, and the last is an empty dashed frame with a question mark in the middle.](/figures/elimination-leaves-a-blank-en.svg)
