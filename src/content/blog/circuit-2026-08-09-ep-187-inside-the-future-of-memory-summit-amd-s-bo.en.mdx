...
![Two rows of parking spaces compared: past years show scattered empty bays, this year every bay is taken, and arrows on the right point to two alternative lots off site.](/figures/parking-lot-full-2026-en.svg)
...
![A hype line spikes high, falls to a trough, then climbs again from that trough, with the earlier stretch in grey and the latest one in colour.](/figures/cxl-hype-forgotten-useful-en.svg)
...
![A straight path along the top is blocked by a wall, while a lower path runs through retirement, removal and requalification to reach the rack, with a return arc looping back to the start.](/figures/buy-new-vs-reuse-loop-en.svg)
...
![A row of bars where the leftmost three stand far above the rest, the fourth is moderate and all the others sit near the floor, showing how unevenly old memory stock is held.](/figures/reuse-benefit-by-inventory-en.svg)
...
![Two columns compare the same three things: customer blocks go from two to eight, the price bar goes from low to high, and the term bar goes from short to long.](/figures/long-contract-then-now-en.svg)
...
![Four ideas side by side each send a line down and converge into a single node at the bottom, forming a fan that closes.](/figures/shared-premise-one-bet-en.svg)
