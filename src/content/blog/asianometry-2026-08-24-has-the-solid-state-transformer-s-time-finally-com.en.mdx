...
Asianometry's 24 August 2026 episode traces the solid state transformer from William McMurray's 1968 GE patent through fifty-eight years of near misses. In the train era ABB demonstrated the PETT in 2007 and by 2012 reached 95–96% efficiency at half the size, yet more than twenty prototypes led to zero commercial deployment because they were too complex and over 50% more expensive; in the grid era GridCo burned $54 million and closed in 2018. This time the buyer is different: an AI rack at 1 MW may need up to 200 kilograms of copper busbars under the 54-volt architecture, and the move to 800 volts DC gives the solid state transformer a real slot. The limit: today's advantages come largely from simulations, with no real operating data yet, and it still has to beat hybrid transformers.
...
![On the left a large block is the core that must be big at 50/60 Hz; on the right the same function shrinks to a small core flanked by two power semiconductors; below, two waveforms, one sparse and one dense, with the dense one matching the small core.](/figures/sst-frequency-for-volume-en.svg)
...
![On the left, four stacked blocks of technical achievement only reach half height; on the right, a single block standing for "50% more expensive" rises far above them, both on the same baseline.](/figures/sst-lost-to-fifty-percent-en.svg)
...
![The top row is a chain of six blocks; the bottom row runs from the same start to the same end with only three blocks, the four removed ones drawn as empty dashed boxes.](/figures/sst-chain-shortened-en.svg)

![Two copper busbars carrying the same 1 MW side by side: the 54-volt one on the left is thick and wide, the 800-volt one on the right is a thin strip.](/figures/sst-copper-busbar-en.svg)

![A timeline with three arcs that rise and fall back to the baseline, and a fourth segment that rises and keeps going, ending in a question mark.](/figures/sst-fifty-eight-years-en.svg)
...
![A four-quadrant chart with the top-right cell highlighted; smart grid sits in the lower left, trains low in the middle, and data centers in the top-right cell.](/figures/sst-buyer-quadrant-en.svg)
...
![Two boxes side by side: the left is filled with five bars, the right has only one short bar and is otherwise blank.](/figures/sst-two-columns-en.svg)
