...
![Two crossing lines: file size falls from upper left to lower right while errors and lost context rise from lower left to upper right, so squeezing harder pulls the two apart.](/figures/quantization-tradeoff-en.svg)
...
![Three bars of very different heights represent three compressed builds of one model, with three dashed lines for graphics card capacity ceilings; the tallest bar passes through all of them.](/figures/vram-ceiling-vs-model-size-en.svg)
...
![A four-quadrant chart: the horizontal axis is how long you'll wait after pressing go, the vertical is whether it must stay on. The low-power mini PC sits top left, the discrete graphics card bottom right, each need in its own quadrant; the bottom-left cell — occasional use, no rush — holds the most people yet no review talks about it](/figures/local-ai-hardware-worth-it-01-en.svg)
