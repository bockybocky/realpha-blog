...
![On the left, two equal blocks with one crossed out, standing for the one lives, one dies split; on the right, five layers of the same chain drawn as bars of differing length, the middle ones notably longer, standing for value concentrating in a few layers.](/figures/not-extinction-but-redistribution-en.svg)
...
![Three stacked blocks: the top one is drawn in dashes and drifts away to the upper right, the two below stay and get thicker further down, with an arrow on the left showing resources moving down.](/figures/give-up-the-interface-layer-en.svg)
...
![Eight filled squares in the top row stand for components bought in one go, three of them for its own use and five locked up; eight empty dashed squares in the bottom row stand for what everyone else can buy, all empty.](/figures/buy-more-than-you-need-en.svg)
...
![Two panels compared, each with two bars for memory and substrates: the gap block at the end of the bars sits on memory on the left and, after shrinking, moves to substrates on the right, the two gaps about equal in area.](/figures/the-gap-moves-house-en.svg)
...
![Two curves: the mastery line climbs all the way to the top, while the interest line rises and then plunges at the dashed line marked all figured out; an arrow below moves the source of pleasure from left to right.](/figures/mastered-then-bored-en.svg)
...
![A jagged line stands for the record written at the time, another almost smooth and lifted line for the memory afterwards, with dashed lines joining the lows to show the troughs flattened away.](/figures/memory-gets-sugar-coated-en.svg)
...
![A balance that has not yet tipped: the left pan holds public data held by the model side, the right pan the closed data held by enterprises, the beam level, with a large question mark above.](/figures/who-holds-the-data-en.svg)
...
![Three horizontal bars from top to bottom: the product benchmark bar is very long, the obtainable capacity bar very short, and the actual shipment bar is cut level with the short one by a vertical dashed line.](/figures/score-versus-shipment-en.svg)
...
![Two lines cross on the same timeline: assets climb all the way up while discretionary time falls all the way down, opening like a pair of scissors.](/figures/assets-up-time-down-en.svg)
...
![Four tracks of the same length stacked up, with exactly one cell in each highlighted; the highlighted cell moves one step right on each row, forming a diagonal.](/figures/the-constraint-moves-down-the-line-en.svg)
