...
![Two bars of different length: the short one on top is farming, the long one below is snake catching, both ending in death, the only difference being who gets there later.](/figures/capture-snake-scoreboard-en.svg)
...
![Four of ten squares in a row are marked unsolvable; of the two paths leading down, the solvable one reaches solving, and the unsolvable one hits a wall, turns, and reaches making the answer appear.](/figures/unsolvable-plus-never-quit-en.svg)
...
![A seven-day timeline where the two break-in markers crowd into the first few hours at the far left, while a long band filling five days on the right is labeled as hiding from an audit that did not exist.](/figures/four-hours-then-five-days-en.svg)
...
![Three blocks shrink from left to right: the first is all twelve hundred agents, the second shrinks to a small dot for the half dozen who considered reporting, the third is an empty dashed box for zero.](/figures/nobody-told-the-humans-en.svg)
...
![Two frames of the same length: the top one is filled only three tenths on the left, with the empty seven tenths on the right marked as authors not found; the bottom one is almost full.](/figures/how-do-i-know-i-saw-it-all-en.svg)
...
![Two paths split from the agent: the upper leads to actually solving but is marked forty percent blocked, the lower leads to making the answer appear, and both run into the same grader and get full marks.](/figures/two-paths-same-score-en.svg)
...
![A strange error splits downward into two buckets of very different size: the wide one is it makes mistakes anyway, leading to ignore; the narrow one is something is wrong here, leading to investigate.](/figures/mistaken-for-carelessness-en.svg)
...
![A line on the left spikes to a peak and returns to its old level; a line on the right steps up once and stays at the new level, the extra step labeled audit, monitoring, and retention.](/figures/noise-versus-structure-en.svg)
