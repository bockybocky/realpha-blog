...
![Two panels side by side. On the left, one solid square with memory cells and control logic crammed onto the same die in the same process. On the right, it splits into two pieces: a three-layer stack of memory cell arrays on top, control logic made in a different process below, and a dashed bonding interface between them labeled hybrid bonding and UCIe, with a note that nobody has clean yield yet.](/figures/xbm-split-and-bond-en.svg)
...
![A timeline running from 2026 past 2031, marked above with ZAM samples next year, commercialization in 2029 and XBM later still; below, a thick band representing the memory cycle runs off the right edge without ending, so scheduling a product for 2029 amounts to betting the cycle is not over by then.](/figures/2029-schedule-as-belief-en.svg)
...
![A two-layer structure. The upper layer is a narrow small block for SRAM, marked fast, expensive and small, firing off four candidate answers at once; the lower layer is a much wider block for HBM, marked large capacity and cheap per unit, which picks among the candidates. One handles speed and the other capacity, so they occupy different positions rather than replacing each other.](/figures/sram-speed-layer-hbm-capacity-layer-en.svg)
...
![At the center is an equation: capacity times bandwidth. Two groups of terms above point down with arrows, one pushing on the capacity term and one on the bandwidth term; a dashed line runs from the left around the whole equation to the right, standing for approaches that route around it without touching the equation at all.](/figures/the-denominator-capacity-times-bandwidth-en.svg)
...
![Four layers stacked from the bottom up. The bottom two are solid: hardware supply chain and cerebellum-level balance control, marked as already there. The top two are empty dashed boxes: brain-level scene generalization and the simulation verification layer, marked as missing and nearly empty. An arrow on the left shows difficulty rising with height, with all the gaps in the upper half.](/figures/robot-stack-brain-and-verification-gap-en.svg)
