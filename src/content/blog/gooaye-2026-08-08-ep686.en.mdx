...
![Three cards in a row for the grandfather's generation, the father's and this one, each carrying a different greeting, all three linked down to one band running underneath marked the same care: the wording changed three times, the thing underneath did not.](/figures/three-generations-same-care-en.svg)
...
![Bars running down from one starting line: two solid bars on the left are the fall caused by July's crowded positioning, and a much longer dashed bar on the right ends in a question mark for the unknown size of a real break in fundamentals.](/figures/july-was-a-rehearsal-en.svg)
...
![Two price lines, one already down at the bottom and one still high; an arrow jumps from the low point of the fallen line to the high line, which then falls too, so whoever switched took both legs down.](/figures/fastest-hands-hurt-most-en.svg)
...
![Two pairs of bars above and below a zero line: in the bull year your bar matches the index and the gap is zero, while in the bear year your bar is far shallower than the index and the gap is marked, showing the gap is what stock picking produced.](/figures/alpha-versus-beta-gap-en.svg)
...
![A price line crosses up through a horizontal dashed line marking the prior high; the shaded stretch before the crossing is the gain you give up, the crossing point is marked act, and the line runs on above it.](/figures/new-high-as-trigger-en.svg)
