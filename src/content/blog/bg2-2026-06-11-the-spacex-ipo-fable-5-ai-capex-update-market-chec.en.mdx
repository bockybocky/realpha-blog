...
![A wide base carries three blocks side by side above it, with three arrows pointing up from the base, showing that all three stand on the same foundation.](/figures/launch-is-the-foundation-en.svg)
...
![A very tall bar on the left and an almost flat bar on the right show the same build cost spread over one flight versus forty.](/figures/amortise-over-one-or-forty-en.svg)
...
![Two fractions side by side. The numerator blocks are the same size, but the denominator block on the right is three times longer, so the multiple it produces is much smaller.](/figures/denominator-rewritten-en.svg)
...
![An arrow running left to right links four nodes, with a vertical line in the middle; the last node sits in a dashed area on the right of that line, showing the bottleneck moving to the side a screener cannot see.](/figures/bottleneck-crosses-the-screen-en.svg)
