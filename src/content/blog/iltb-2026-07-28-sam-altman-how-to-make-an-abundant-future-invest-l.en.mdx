...
![The same three-layer stack of research idea, compute and data repeats four times, with a different layer drawn narrow each time, showing the bottleneck moving along the timeline.](/figures/bottleneck-moves-en.svg)
...
![A steeply rising training-spend line, with the inference-revenue line splitting midway into one path that keeps up and one that does not.](/figures/inference-vs-training-slope-en.svg)
...
![Two stacked layers of blocks: the upper is the intelligence itself, which gets commoditised, and the lower is the compute and workflow that stay, with a line between them marking where the premium should be paid.](/figures/commoditization-line-en.svg)
...
![Five pillars from different industries stand side by side on one foundation, and that foundation has cracked, showing that what looks diversified is a single position.](/figures/five-pillars-one-assumption-en.svg)
...
![The left panel is a jagged line jumping violently up and down; the right panel is the same period viewed from further away, a smooth rising curve.](/figures/noise-vs-exponential-en.svg)
