...
![A line climbs from banks at lower left up to private credit, outside the backstop and eating its own losses, then slides down to life insurers at lower right, landing lower than it started, with the backstop band on the right turned dashed and marked untested.](/figures/risk-relocation-loop-en.svg)
...
![Three boxes form a triangular loop with arrows joining buyouts, private credit and the life insurer end to end, all enclosed by a dashed frame marking them as parts of the same private equity group.](/figures/insurance-flywheel-en.svg)
...
![Two timelines compared: on the left a run of regular payments before failure builds into a fund column, while on the right there is nothing before failure and only afterwards do arrows flow in from surviving peers and branch on to taxpayers.](/figures/two-backstops-funding-en.svg)
...
![Two identical distribution curves side by side: the cap line on the left sits at the far right and covers almost the whole area, while the cap line on the right sits well to the left and leaves a large area beyond it uncovered.](/figures/guarantee-coverage-gap-en.svg)
