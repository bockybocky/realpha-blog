---
title: "生成式 AI 認證 考前重點總整理（完整版）：71 個關鍵名詞速查 ＋ 四大面向逐章考點"
description: "資策會生成式 AI 能力認證的考前速查工具：71 個關鍵名詞完整速查表（分五組，每個附「怎麼考」），加上基礎知識／Prompt／應用技能／倫理法律四大面向、逐章的高頻考點與「看到 X 選 Y」對應。搭配我的準備方法文一起服用。學習分享，非販售題庫。"
...
tldr: "資策會生成式 AI 能力認證的『考前速查工具』完整版。上半＝71 個關鍵名詞完整速查表（原理架構／Prompt&Agent／工具應用／資料治理MLOps／倫理法律五組，含 RAG、Transformer、RLHF、Diffusion、MoE、Data Drift、同態加密、聯邦學習、Red Teaming、LAWS、EU AI Act…每個附怎麼考）；下半＝四大面向逐章考點（1.1 Scaling Laws/Token/Temperature、1.3 參數vs超參數/CLM vs MLM、2.2 Data Drift vs Concept Drift/RBAC vs ABAC、3.4 TTS/STT/S2S、4.4 PETs 隱私增強技術家族…），每組附『看到 X 選 Y』的辨析鉤。全部概念層、不含任何真題。搭配同站『我怎麼用 AI 協作準備』方法文一起看。"
---
...
![三根高度懸殊的柱子由左到右快速變矮，分別是預訓練、微調、RAG，柱高代表運算量，說明三者代價差一個量級、改動的東西也不同。](/figures/pretrain-finetune-rag-cost.svg)
...
![一個大方框裡裝著兩個小方框，大框是提示注入，小框分別是越獄與提示洩漏，用包含關係說明越獄只是注入的一種。](/figures/prompt-injection-contains-jailbreak.svg)
...
![三個畫布並排：第一個在內部挖出一塊虛線空缺，第二個在原畫布外圍多出虛線邊界，第三個由小畫布放大成大畫布，用邊界位置區分三種修圖。](/figures/inpaint-outpaint-upscale.svg)
...
![左格的資料點整團往右移但分界線不動，右格的資料點原地不動但分界線轉了個角度，用「誰在動」區分兩種漂移。](/figures/data-drift-vs-concept-drift.svg)
...
![一座四層金字塔，頂層最窄是不可接受風險、往下逐層變寬到最低風險，上窄下寬同時表示「管得越嚴」與「數量越多」。](/figures/eu-ai-act-risk-pyramid.svg)
...
![左半是兩個方塊互相箭頭形成的循環，右半是四格由滿佈雜點逐漸變乾淨的方框序列，對比對抗式生成與逐步去噪。](/figures/gan-vs-diffusion.svg)
- 必記：**垂直 AI**＝針對特定產業訓練優化。
...
![兩個相同的二乘二方格並排，左邊塗亮整條直行、右邊塗亮整條橫列，說明兩個指標分子相同但分母是不同的一整條。](/figures/precision-recall-denominator.svg)
...
![左圖同一個字只有一個固定位置的點，兩種語境都指向它；右圖同一個字分裂成兩個點，各自靠近河流與金錢，用位置表示語境。](/figures/word2vec-vs-bert-position.svg)
...
![八個專家方格中只有兩格被點亮，右側一高一矮兩根柱子對比總參數與每次實際運算量。](/figures/moe-partial-activation.svg)
...
![一條 U 形曲線，兩端高、中間塌陷，橫軸是對話從開頭到結尾的位置，縱軸是被記得的程度。](/figures/lost-in-the-middle-curve.svg)
...
![上方文字與語音兩個方塊之間有一對相反方向的箭頭，下方語音到語音另有一條箭頭，用箭頭方向區分三個名詞。](/figures/tts-stt-s2s-directions.svg)
...
![五條並排的橫向箭頭從自己這端指向同一個對方，箭頭上運送的東西各不相同：參數、加雜訊的統計、密文、碎片、假資料。](/figures/pets-what-leaves-your-side.svg)
