...
![Two bands compared: the stock price line in the upper band jumps early in the timeline and then flattens, while the good-news bars in the lower band only reach their peak at the end, the two peaks clearly offset on the timeline](/figures/price-runs-ahead-of-news-en.svg)
...
![On the left, three bars rise year by year, but the dark top segment marking the increase over last year first thickens then thins fast; on the right a large box shrinks into a small one](/figures/level-still-high-increment-falls-en.svg)
...
![A solid full bar on the left stands for a steel mill; on the right the top half of the data center bar becomes an empty dashed box after ten years, with only the lower half remaining](/figures/half-the-cost-gone-in-ten-years-en.svg)
...
![From the same purchase point two lines diverge, one for company profit rising steadily, the other for the annualized share return falling steadily](/figures/starting-multiple-curse-en.svg)
...
![A staircase climbing from lower left to upper right, each step higher than the last, with the top two steps in a warning color](/figures/capital-casserole-staircase-en.svg)
...
![Two nearly equal bars on the left are cut level by a horizontal dashed line drawn beforehand, while the share price bar on the right drops a short way below the zero line](/figures/the-line-not-the-height-en.svg)
...
![The top row has four bars of equal height; in the bottom row the first is raised much higher and the following three sag noticeably, with an arc carrying the later volume to the front](/figures/demand-pulled-forward-en.svg)
...
![A line rises to the right for profit growth then falls to the right for multiple compression, ending back on the horizontal dashed line level with the start](/figures/growth-up-multiple-down-en.svg)
...
![A long bar filling the whole frame stands for evolution; the bar for markets below is so short it needs an arrow to point it out](/figures/instinct-versus-market-timescale-en.svg)

![Two trend lines cross in the middle: the one that rose sharply first then flattens, the one that fell sharply first then climbs all the way up](/figures/roles-swapped-crossing-lines-en.svg)
...
![Two panels: on the left the result is above the dashed line but the line slides up to meet it afterwards; on the right the result is below the line and the line slides down to meet it, and both gaps become zero](/figures/the-unwritten-line-moves-en.svg)
