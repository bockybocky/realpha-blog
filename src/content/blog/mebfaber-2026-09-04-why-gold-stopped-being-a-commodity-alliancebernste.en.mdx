...
![A dashed empty slot is left in the commodities box on the left as a block labeled gold moves into the money box on the right, with a horizontal arrow between the two boxes.](/figures/gold-moves-out-of-commodities-en.svg)
...
![A long timeline where most stretches sit above the zero line in the same-direction zone, with only the short span from 1998 to 2021 below it in the opposite-direction zone, returning above after 2022.](/figures/stocks-bonds-same-direction-is-normal-en.svg)
...
![Of eight bars only the leftmost two have height, the other six shrunk to marks on the ground, with a dashed frame above showing where they once stood.](/figures/survivors-only-two-bars-left-en.svg)
...
![Two downward bars below the zero line for demographics and climate subtract one percentage point in total, and one upward bar on the right adds it back to the zero line, ending flat.](/figures/ai-productivity-arithmetic-bridge-en.svg)
...
![In the demand shock panel on the left the stock arrow points down and the bond arrow up, offsetting each other; in the supply shock panel on the right both arrows point down.](/figures/demand-shock-versus-supply-shock-en.svg)
...
![On the left a rising and falling line with a dashed threshold trailing beneath it that it never touches; on the right the same line crossed once by a fixed horizontal line, with the crossing marked by a dot.](/figures/floating-threshold-versus-fixed-threshold-en.svg)
...
![A bar in three segments; the first two, earnings growth and dividends, are solid, while the last, multiple expansion, is drawn in a dashed frame and crossed out.](/figures/valuation-only-caps-one-block-en.svg)
...
![Three boxes on the left labeled job, person, and habit send three lines converging into one circle on the right holding the same single source.](/figures/three-safe-things-one-root-en.svg)