...
![Two bars of equal height stand for the same own capital: with no leverage only a sliver at the top is eaten, while at five times leverage the whole bar is gone, so the difference in height shows the wipeout at a glance.](/figures/leverage-two-limit-downs-en.svg)
...
![Two bars of equal length split the same day's minus seven percent into an index part and a stock part: the top bar is almost all index with a sliver of stock at the right, and the bottom bar is the reverse.](/figures/split-beta-from-alpha-en.svg)
