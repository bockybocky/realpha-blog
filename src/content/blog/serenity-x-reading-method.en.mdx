...
![A filing on the left with two lines branching up and down to two cards, the upper listing bearish phrases such as capacity shortfall and the lower listing tightness phrases such as qualified capacity is scarce, and a line at the bottom reading both are true at once.](/figures/same-filing-two-readings-en.svg)
...
![Two timelines: the upper one is crowded with dozens of ticks, standing for a judgment priced in dollars and tested every day, and the lower one has only four, standing for a judgment written in capacity and schedules that is checked once a quarter.](/figures/unit-decides-falsification-frequency-en.svg)
...
![Three cards on the left each draw a line converging on a round node in the middle marked interconnect is the tightest layer, while a dashed line branches from that node down to the right to a card marked price, three converging against one dissenting.](/figures/three-paths-converge-price-diverges-en.svg)
