...
![One large fraction. The numerator line rises while the denominator line falls, and the ratio they produce breaks out above its normal range on the right.](/figures/numerator-denominator-diverge-en.svg)
...
![Two timelines stacked. The upper one has sparse ticks reaching out years; the lower one has dense ticks with every event crowded into the first few slots.](/figures/two-clocks-different-scales-en.svg)
...
![Two nearly identical curves offset horizontally, the leading one the stock price and the trailing one the contract price, with a dashed box below for the long-term contract price you cannot see.](/figures/price-leads-quote-echo-en.svg)
...
![Two bars of different lengths, each starting with an identically sized dark cost block; the short one is nearly half eaten, the long one loses only a sliver.](/figures/same-fee-spread-over-time-en.svg)
...
![Ten people set out on the left; seven fall into a gray silent zone with no voice, three reach the right carrying speech bubbles, and the right frame holds only those three.](/figures/survivors-are-the-only-speakers-en.svg)
