...
![Two curves from the same starting point, steps on the horizontal axis and the chance of still being on track on the vertical; one drifts down gently, the other falls off a cliff after a few dozen steps, opening an ever-wider gap between them](/figures/error-compounds-over-steps-en.svg)
...
![Three bars of very different lengths, the longest labeled four hundred ninety-five seconds and the shortest fifty-nine; each carries the number of defects found at its right end, two on the longest and one on each of the others](/figures/slow-but-finds-more-en.svg)
...
![A long bar almost entirely empty with only a short stretch filled at the far left, labeled one percent, and a line of small type beside it reading paid quota](/figures/quota-mostly-unused-en.svg)
