...
Mine said one percent, which was a little embarrassing. If yours is in the same neighborhood, don't rush to switch vendors—find one job you repeat every week and hand the whole thing over once, just to see. Look at the sticky note again on the same day next month. If the number moved, you found work that can be handed off. If it didn't, no harm done; it just means the bottleneck isn't the tool yet, it's that we haven't broken the work into a shape that can be handed over. I'm still working on that part myself.
