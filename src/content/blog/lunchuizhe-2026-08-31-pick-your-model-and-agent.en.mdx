...
Before choosing a tool, ask how many steps the task runs. For something that resolves in one step, all three vendors get it right, so pay the cheapest. For something that runs a hundred steps, errors accumulate, and only then is the expensive one worth it.

An exercise for today: open the vendors you pay for, look at how much of this month's quota you have used, and write that number down. If, like me, it is under ten percent, don't rush to switch vendors. Find one job you repeat every week that could be handed over whole, and hand it over once. Check the number again on the same day next month: if it moved, you found work that can be handed off; if it didn't, the bottleneck isn't the tool—and only then will you know which vendor is worth the money.
