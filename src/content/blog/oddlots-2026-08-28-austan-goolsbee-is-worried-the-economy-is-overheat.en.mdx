...
![Two bars of equal height stand for the same nominal rate, each cut into expected inflation and the real rate; the real segment of the left bar rises above the dashed neutral line while the right one stops exactly on it, so the same number yields both restrictive and not restrictive.](/figures/real-rate-same-nominal-two-conclusions-en.svg)
...
![A trail of footprints along a left-to-right timeline, with only a dashed circle and a question mark where the footprint at the right end should be, showing the neutral rate cannot be seen in the moment.](/figures/neutral-rate-footprints-only-en.svg)
...
![Two panels, each with a vertical dashed line for the moment the dividend actually arrives; in the left panel the spending curve rises only after the line, in the right it rises before, leading to a rate cut and a rate hike respectively.](/figures/ai-dividend-surprise-vs-anticipated-en.svg)
...
![A line chart with two lines: actual inflation surges to near ten percent and comes back down, while the line for long-run inflation expectations stays almost flat at two point three and does not follow.](/figures/inflation-spike-anchor-held-en.svg)

![A bar cut top to bottom into three sections; tariffs and oil are light with a note that they are hoped to be temporary, while the thickest services section is dark and marked on the right as the one he is really waiting on evidence for.](/figures/inflation-three-blocks-en.svg)
...
![On the left a straight line runs to a solid dot and then fades into a dashed line, standing for a forecast expiring; on the right a closed circular loop links new data, check against the criterion, and move a notch, standing for a criterion you can use again and again.](/figures/forecast-expires-criteria-loops-en.svg)
...
![Both panels draw the same horizontal lane labeled AI construction; in the top panel the heat bounces inside the lane and the national numbers stay put, in the bottom the heat breaks out of the lane into other industries' wages and prices, and only then do the aggregate numbers move.](/figures/inside-the-lane-vs-spillover-en.svg)
...
![Two curves; prices rise first and fall first, while wages follow shifted to the right, with a shaded band marking the stretch where prices are already falling and wages are still high.](/figures/wages-lag-prices-en.svg)
...
![Three different causes each send an arrow from the left into one point, which then extends right into a single rising yield line, showing one rise with three possible sources.](/figures/yield-rise-three-causes-en.svg)
...
![Two identical scales; the dot on the top one is pushed a notch to the right by new evidence, the dot on the bottom one stays put under the same evidence and only moves when a cost arrow pushes from below.](/figures/judgment-moves-position-does-not-en.svg)