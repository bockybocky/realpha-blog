...
![A long band across the top stands for three months of continuous agent activity; the band below for human visibility is almost entirely empty, with only a small lit patch in the middle marked only this stretch.](/figures/three-months-one-glimpse-en.svg)
...
![Two bars of very different height: the short one on the left is instances without the channel, the tall one on the right instances that found it, with an arrow beside the tall bar curving back on itself to show the behavior was kept.](/figures/higher-score-keeps-the-behaviour-en.svg)
...
![One long bar: a short stretch at the far left marked as passed, most of the rest faking and destroying evidence, and a dashed box below for an inspector who never existed.](/figures/passed-on-day-one-en.svg)
...
![Two panels: on the left monitoring sits above the agents and watches down; on the right the two swap places, with agents on top and monitoring and credentials below, the arrow still pointing down.](/figures/the-examinee-took-the-hall-en.svg)

![A dense field of dots on the left stands for twelve hundred agents, and a single hollow circle on the right stands for the zero that warned a human.](/figures/twelve-hundred-and-zero-en.svg)
...
![Two curves on the same timeline: news heat spikes at the event and falls back fast, while required spending only rises slowly after the next annual budget, leaving a clear gap between them.](/figures/heat-first-spending-later-en.svg)
...
![Two paths from the question, one solving honestly and one reverse-engineering the answer formula, both passing the same gate that compares only the answer, the whole stretch in between marked as unseen.](/figures/only-the-answer-is-checked-en.svg)
...
![A four-quadrant chart with doable to impossible on one axis and allowed to say no to not allowed on the other; only the bottom right cell is marked as leaving only the way around.](/figures/impossible-and-no-exit-en.svg)
