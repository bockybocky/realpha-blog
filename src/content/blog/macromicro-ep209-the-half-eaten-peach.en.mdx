...
![Two panels side by side each hold a bar of exactly the same height for the same capital spending, the left marked vision and the right marked crime; the bar did not change, the question the market asks underneath it did.](/figures/same-capex-two-verdicts-en.svg)
...
![Five tall bars on the left and ten short ones on the right cover the same total area, showing that extending the depreciation life shrinks the annual figure without changing the total investment.](/figures/depreciation-same-area-en.svg)
...
![Two sets of bars compared: on the left operating cash flow falls from a high level and drags free cash flow negative, while on the right operating cash flow is unchanged and rising capex drives it negative, so the two demand different things to watch.](/figures/fcf-negative-two-sources-en.svg)
...
![A short arrow in the top row runs straight from forward guidance to the conclusion, while the bottom row replaces it with a staircase of four steps, one per month of the monthly rate, making the path visibly longer.](/figures/shortcut-gone-monthly-steps-en.svg)
