...
![Two lines run from 2010 to 2026: the inflation expectation line moves almost flat inside a narrow gray band, while the real rate line climbs from its lowest point to its highest, showing that what moved was the real rate, not inflation expectations.](/figures/yield-split-real-vs-inflation-en.svg)
...
![Two bars split by a zero line: the left one points up for the positive returns of the 2010s, the right one points down for the negative returns of the 2020s, about twice as long.](/figures/hedge-two-decades-en.svg)
...
![One flow of money splits from the left into two branches: the upper branch thickens as it flows into corporate bonds, the lower thins as it flows into Treasuries, where an upward arrow shows the yield being pushed higher.](/figures/bond-demand-split-en.svg)
...
![Two lines meet at the same point on the right, one falling all the way from a high level, one rising all the way from a low one, and the meeting point is the same percentage.](/figures/two-kinds-of-five-percent-en.svg)

![Two panels: on the left arrows point out from the company to shareholders; on the right they reverse, running from the market into the company and then all into a single large block.](/figures/cashflow-direction-flip-en.svg)

![Two paths climb from the same start to the same horizontal line, one steep and quickly at the top before flattening, one rising slowly; the horizontal line itself is also marked as a threshold.](/figures/speed-and-level-en.svg)
...
![Two panels each with a line: the left one oscillates in small swings around a center line, the right one leaves the center line downward and never returns.](/figures/two-ways-a-hedge-loses-en.svg)
...
![Two groups of bars compared: in each, a gray segment on the right covers part of the bar and the bright remainder is the denominator; the lower group is longer overall but covered more, so what is left is shorter.](/figures/denominator-tug-of-war-en.svg)
...
![Three bars side by side: the first and tallest is earnings upgrades, the second is short and points down for multiple compression, the third is the index gain left after the two offset.](/figures/return-decomposition-en.svg)
...
![A wide band at the top narrows sharply after passing three gates into a thin strip, while a second band below grows wider as it goes, the two running in opposite directions.](/figures/conditions-multiply-en.svg)
...
![Four pillars stand side by side on the same floor, with a downward arrow beneath it, suggesting that if the floor drops all four move together.](/figures/one-shared-foundation-en.svg)
...
![Two bars of equal length each stand for an index: the top one is filled nearly half way, the bottom one only a short stretch, the fill being the weight of tech](/figures/weighting-concentration-en.svg)
