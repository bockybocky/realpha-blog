...
def main() -> int:
    ...
    if not newly:
        ...
        if "human_reminded=0" not in res.stdout:
            notify("📒 驗證簿有人工卡到期待裁決", f"settle 輸出：{res.stdout.strip()}\n請查 inbox 的 ledger_*.md 依判準裁決。")
    ...
    if args.no_publish:
        notify("📒 驗證簿開獎（未發布，--no-publish）", body)
